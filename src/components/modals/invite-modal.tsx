	...
	return (
		<Dialog open={isModalOpen} onOpenChange={onClose}>
			<DialogContent className="bg-white  dark:bg-black dark:text-white text-black p-0 overflow-hidden">
				...
				<div className="p-6">
					<Label className="uppercase dark:text-white/70 text-xs font-bold text-zinc-500 ">Server Invite Link</Label>
					<div className="flex items-center mt-2 gap-x-2">
						<Input disabled={isLoading} className="bg-zinc-300/50! dark:bg-neutral-700! focus-visible:ring-0 border-0 focus-visible:ring-offset-0"
							value={inviteUrl}
							onChange={() => { }}
						/>
						<Button disabled={isLoading} className="dark:bg-neutral-300" onClick={onCopy} size="icon">
							<CopyLabel className="size-4" />
						</Button>
					</div>
					<Button disabled={isLoading} onClick={onNew} variant="link" size="sm" className="text-xs dark:text-white/70 text-zinc-500 mt-4">
						Generate a new link
