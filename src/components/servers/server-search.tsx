...
export const ServerSearch = ({ data }: ServerSearchProps) => {
	...
	const onClick = ({ id, type }: { id: string, type: "channel" | "member" }) => {
		setOpen(false);
		if (type === "member") {
			return router.push(`/api/server/${params?.serverId}/conversations/${id}`)
		}
		if (type === "channel") {
	...
	return (
		<>
			<button onClick={() => setOpen(true)} className="group px-2 py-2   cursor-pointer rounded-md flex items-center gap-x-3 w-full hover:bg-zinc-700/10!   dark:hover:bg-zinc-700/50! transition">
				...
				<kbd className="pointer-events-none inline-flex h-5 items-center gap-1 rounded border bg-muted px-1.5 font-mono text-[10px] text-muted-foreground ml-auto">
